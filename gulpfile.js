...
//===
function cleanDist() {
  return src(['static/img', 'static/css', 'static/fonts'])
    .pipe(clean());
}
...
// JS vendors
function jsVendors() {
  return src(SRC_PATH + '/vendors/**/*.js')
    .pipe(dest(DEST_PATH + '/vendors/'))
    .pipe(browserSync.stream());
}

function mjml() {
  return src('app/templates/emails/*.mjml')
    .pipe(exec(file => `mjml ${file.path} -o ${file.path.split('.')[0]}.html`))
    .pipe(browserSync.stream());
}
...
const build = series(
  cleanDist,
  parallel(sassCompile, typescript, jsVendors, images, fonts, buildFontIcons, mjml)
);
...
exports.dev = function () {
  ...
  watch([SRC_PATH + '/vendors/*', SRC_PATH + '/vendors/**/*.js'], jsVendors);
